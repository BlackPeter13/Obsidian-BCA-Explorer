...
namespace NBXplorer.Logging
{
	...
	public class CustomerConsoleLogger : ILogger
	{
		...
		internal CustomerConsoleLogger(string name, Func<string, LogLevel, bool> filter, IExternalScopeProvider scopeProvider, ConsoleLoggerProcessor loggerProcessor)
		{
			...
			// Single cross-platform console backend (the old Windows/Ansi
			// split relied on ASP.NET Core internal types, gone since 3.0).
			Console = new SimpleSystemConsole();
		}
		...
		public class SimpleSystemConsole : IConsole
		{
			...
			public void Write(string message, ConsoleColor? background, ConsoleColor? foreground)
			{
				lock(_lock)
				{
					...
					{
						try { System.Console.BackgroundColor = background.Value; } catch { }
					}
					if(foreground.HasValue)
					{
						try { System.Console.ForegroundColor = foreground.Value; } catch { }
					}
	...
	public class ConsoleLoggerProcessor : IDisposable
	{
		...
		public CustomerConsoleLogger.IConsole Console;
