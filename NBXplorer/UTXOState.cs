...
namespace NBXplorer
{
	...
	public class UTXOState
	{
		...
		public ApplyTransactionResult Apply(TrackedTransaction trackedTransaction)
		{
			...
			{
				if(spentOutpoint.Hash == uint256.Zero && spentOutpoint.N == uint.MaxValue)
					continue; // Coinbase input: mints coins, spends nothing, never conflicts
			...
			{
				if (spentOutpoint.Hash == uint256.Zero && spentOutpoint.N == uint.MaxValue)
					continue; // Coinbase input: mints coins, spends nothing
