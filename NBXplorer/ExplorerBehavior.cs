...
namespace NBXplorer
{
	public class ExplorerBehavior : NodeBehavior
	{
		...
		private async Task SaveMatches(Block block)
		{
			...
			// BCA PoS-flagged blocks: P2P serves legacy 80-byte headers, so the parsed
			// id differs from the canonical (RPC-synced) id the fetch was keyed by.
			// Resolve via prev-hash linkage so in-flight completion still matches.
			if (!_InFlights.ContainsKey(blockHash))
			{
				var prev = Chain.GetBlock(block.Header.HashPrevBlock);
				var canonical = prev == null ? null : Chain.GetBlock(prev.Height + 1);
				if (canonical != null && _InFlights.ContainsKey(canonical.Hash))
